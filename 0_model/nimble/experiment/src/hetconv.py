...
class HetConv(nn.Module):
    def __init__(self, in_channels, out_channels, p):
        ...
        print(in_channels, out_channels, 3)
        self.gwc = nn.Conv2d(in_channels, out_channels,
                             kernel_size=3, padding=1, groups=p, bias=False)
        # Pointwise Convolution
        self.pwc = nn.Conv2d(in_channels, out_channels,
...
class vgg16bn(nn.Module):
    def __init__(self, f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, f13, p):
        ...
        self.features = nn.Sequential(
            ...
            # HetConv(f10, f11, p),
            # nn.BatchNorm2d(f11),
            nn.Conv2d(3, f11, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            HetConv(f11, f12, p),
            nn.BatchNorm2d(f12),
            nn.ReLU(inplace=True),
...
def main():
    ...
    # By changing "part" P value, You can reproduce the results for VGG-16 on CIFAR-10.
    part = 1

    model = vgg16bn(64, 64, 128, 128, 256, 256, 256,
                    512, 512, 512, 512, 512, 512, part)
    model = model.cuda()
...
#     convlist = [model.features[3], model.features[7], model.features[10], model.features[14], model.features[17], model.features[20],
#                 model.features[24], model.features[27], model.features[30], model.features[34], model.features[37], model.features[40]]
