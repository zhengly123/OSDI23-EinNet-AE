...
cudnnMathType_t MATH_TYPE = CUDNN_FMA_MATH;
...
int main(int argc, const char *argv[]) {
    ...
    checkCUDNN(cudnnSetTensor4dDescriptor(output_descriptor,
                                          ...
                                          /*width=*/OUTPUT_WIDTH));
    auto best_algo = choose_algo;
    int rounds = 100, warmup = 50;
    double time_conv = 0;
    for (int i = 0; i < rounds; ++i) {
        size_t workspace_bytes = 0;
        auto status((cudnnGetConvolutionForwardWorkspaceSize(
            cudnn, input_descriptor, kernel_descriptor, convolution_descriptor,
            output_descriptor, (cudnnConvolutionFwdAlgo_t)best_algo,
            &workspace_bytes)));
        ...
        // only profile at last iteration
        if (i == rounds - 1)
            cudaProfilerStart();
        {
            HANDLE_ERROR(cudaDeviceSynchronize());
            ...
            auto status(cudnnConvolutionForward(
                ...
                (cudnnConvolutionFwdAlgo_t)best_algo, d_workspace,
    ...
    std::cout << "Time: " << time_conv / (rounds - warmup) << " ms" << std::endl;
    std::cout << "Algo: " << algo_name[best_algo] << std::endl;
